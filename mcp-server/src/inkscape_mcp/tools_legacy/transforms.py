...
SUPPORTED_INTERPOLATION = {"none", "linear", "cubic", "lanczos", "nohalo", "lohalo"}
        ...
        async def crop_image(
            ...
            try:
                ...
                if x + width > original_width:
                    return self.create_error_response(
                        f"Crop area extends beyond image width ({original_width}px)"
                    )

                if y + height > original_height:
                    return self.create_error_response(
                        f"Crop area extends beyond image height ({original_height}px)"
                    )
...
"""
                ...
                crop_data = {
                    ...
                    "cropped_dimensions": {"width": width, "height": height},
                    "output": {
                        "path": str(output_path_obj.resolve()),
                        "size_bytes": output_stat.st_size,
                    },
                ...
                return self.create_success_response(
                    data=crop_data, message=f"Image cropped to {width}x{height} pixels"
                )
...
"""
                ...
                rotate_data = {
                    "original_dimensions": {"width": original_width, "height": original_height},
                    "rotation": {"degrees": degrees, "fill_color": fill_color},
                    "output": {
                        "path": str(output_path_obj.resolve()),
                        "size_bytes": output_stat.st_size,
                    },
                ...
                return self.create_success_response(
                    data=rotate_data, message=f"Image rotated by {degrees} degrees"
                )
...
"""
                ...
                flip_data = {
                    ...
                    "operation": {
                        ...
                        and output_format in ["jpg", "jpeg", "tiff", "tif", "png", "webp"],
                    },
                    "original_dimensions": {"width": original_width, "height": original_height},
                    "output": {
                        "path": output_abs,
                ...
                return self.create_success_response(
                    data=flip_data, message=f"Image flipped {direction}ally"
                )
