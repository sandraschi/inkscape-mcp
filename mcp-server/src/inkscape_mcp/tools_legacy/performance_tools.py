    ...
    def register_tools(self, app: FastMCP) -> None:
        ...
        async def get_performance_metrics(
            ...
            try:
                ...
                if operation_type and operation_type not in self._performance_metrics:
                    return self.create_error_response(
                        f"Operation type '{operation_type}' not found"
                    )
                ...
                if filtered_metrics:
                    processing_times = [
                        m.get("processing_time", 0) for m in filtered_metrics.values()
                    ]
        ...
        async def get_system_performance_info() -> Dict[str, Any]:
            ...
            try:
                ...
                self.logger.error(
                    f"System performance info retrieval failed: {str(e)}", exc_info=True
                )
                return self.create_error_response(
                    f"System performance info retrieval failed: {str(e)}"
                )
