...
def log_tool_result(context: Dict[str, Any], result: Any, success: bool = True) -> None:
    ...
    log_func(
        ...
        extra={
            "tool_result": {**context, "end_time": end_time, "result": result, "status": status}
        },
    )
...
def log_tool_registration(category: str, tool_count: int, success: bool = True) -> None:
    ...
    logger.info(
        ...
        extra={
            "tool_registration": {"category": category, "tool_count": tool_count, "status": status}
        },
    )
...
def log_config_load(config_path: Optional[Path], success: bool) -> None:
    ...
    if success:
        logger.info(
            ...
            extra={
                "config_load": {"path": str(config_path) if config_path else None, "success": True}
            },
        )
    else:
        logger.error(
            ...
            extra={
                "config_load": {"path": str(config_path) if config_path else None, "success": False}
            },
        )
