    ...
    def register_tools(self, app) -> None:
        ...
        async def merge_layers(
            ...
            try:
                ...
                for idx in layer_indices:
                    set_active_layers += (
                        f"(gimp-image-set-active-layer image (aref layer-array {idx}))\n"
                    )
            ...
            try:
                ...
                if mask_type == "color" and color.startswith("#"):
                    ...
                    if len(hex_color) == 3:  # Short format (#RGB)
                        ...
                        color_values = [int(hex_color[i] * 2, 16) for i in range(3)] + [
                            int(hex_color[3] * 2, 16)
                        ]
                    elif len(hex_color) == 6:  # Full format (#RRGGBB)
                        color_values = [int(hex_color[i : i + 2], 16) for i in range(0, 6, 2)] + [
                            255
                        ]
        ...
        async def toggle_layer_visibility(
            ...
            try:
                ...
                if not result or "Error" in result:
                    return self.create_error_response(
                        f"Failed to toggle layer visibility: {result}"
                    )
        ...
        async def set_layer_opacity(
            ...
            try:
                ...
                if not layer_indices and not affect_others:
                    return self.create_error_response(
                        "No layer indices provided and affect_others is False"
                    )
            ...
            try:
                ...
                try:
                    ...
                    for item in result_str[2:-2].split(")("):
                        ...
                        new_names = [
                            name.strip('"') for name in " ".join(parts[-1:])[3:-1].split('" "')
                        ]
                    ...
                    self.logger.error(
                        f"Error parsing duplication result: {e}\nRaw result: {result}",
                        exc_info=True,
                    )
