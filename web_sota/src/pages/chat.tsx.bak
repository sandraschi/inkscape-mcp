  ...
  const send = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setMessages((m) => [
          ...m,
          { role: "assistant", text: `Error: HTTP ${res.status}` },
        ]);
        return;
      }
      setMessages((m) => [
    ...
    <div className="flex h-[calc(100vh-8rem)] flex-col space-y-4">
      <div>
        ...
        <p className="text-slate-400">
          Not a local LLM chat. Messages call{" "}
          <code className="text-slate-300">POST /api/chat</code> and log
          server-side. For vector work, use MCP in your IDE.
        </p>
      </div>
